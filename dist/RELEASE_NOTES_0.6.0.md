...
- Nowa warstwa wielu silnikĂłw gĹ‚osu w Studio Syntezy.
- XTTS v2 pozostaje stabilnym, domyĹ›lnym silnikiem dla jÄ™zyka polskiego.
- F5-TTS v1 moĹĽna zainstalowaÄ‡ z GUI do izolowanego `engine_runtime/f5`; inference dziaĹ‚a jako osobny proces i nie modyfikuje gĹ‚Ăłwnego venv SoundCore.
- Qwen3-TTS 0.6B Base moĹĽna zainstalowaÄ‡ z GUI do izolowanego `engine_runtime/qwen`; SoundCore wymaga osobnego Python 3.12 i uĹĽywa trybu voice-clone x-vector/Auto jako eksperymentu.
- CosyVoice 3 jest widoczny w laboratorium jako wariant eksperymentalny/manualny; oficjalny stack nie deklaruje jÄ™zyka polskiego.
- Aktualizator chroni `engine_runtime`, wiÄ™c pobrane Ĺ›rodowiska/model cache nie sÄ… usuwane przy aktualizacji SoundCore.
- Studio ma wybĂłr silnika oraz A/B pomiÄ™dzy: XTTS bazowy, XTTS wytrenowany, F5-TTS i Qwen3-TTS.
- Historia syntezy zapisuje nazwÄ™ uĹĽytego silnika.

## RVC Manager

- Nowy panel RVC w `Profile gĹ‚osu`.
- Instalacja `rvc-python` bez rÄ™cznego terminala.
- WybĂłr modelu `.pth` i opcjonalnego pliku `.index` z GUI.
- Status runtime/modelu per profil oraz moĹĽliwoĹ›Ä‡ odpiÄ™cia RVC.
- Po podpiÄ™ciu modelu checkbox `Popraw barwÄ™ przez RVC` dziaĹ‚a jako post-processing wyniku dowolnego dostÄ™pnego silnika TTS.

## StabilnoĹ›Ä‡

- Eksperymentalne silniki sÄ… izolowane od Ĺ›rodowiska XTTS.
- Zachowano wszystkie poprawki 0.5.5 dotyczÄ…ce bezpiecznego, pojedynczego odtwarzacza A/B.

# SoundCore 0.5.5

## Stabilne odtwarzanie A/B

- Jeden zarzÄ…dzany odtwarzacz audio dla caĹ‚ej aplikacji.
- Nowy odsĹ‚uch A/B, historii, profilu lub ostatniej syntezy bezpiecznie zatrzymuje poprzedni.
- UsuniÄ™to rĂłwnolegĹ‚e `sd.play()/sd.wait()` z wielu wÄ…tkĂłw, ktĂłre mogĹ‚y crashowaÄ‡ PortAudio/aplikacjÄ™.
- PrzeĹ‚Ä…czanie A â†” B jest responsywne i nie tworzy nakĹ‚adajÄ…cych siÄ™ strumieni.
- BĹ‚Ä™dy urzÄ…dzenia audio podczas samego odsĹ‚uchu nie wywracajÄ… procesu GUI.
