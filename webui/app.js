...
async function loadSynthesisHistory(){
  ...
  try{const r=await api('list_synthesis_history',120);const rows=r.items||[];$('synthesisHistory').innerHTML=rows.length?rows.map(x=>`<div class="synth-history-row"><div><b>${esc(x.profile)} <span class="model-chip">${x.model_mode==='trained'?'WYTRAINOWANY':'BAZOWY'}</span></b><small>${esc(x.created_at||'')} · ${Number(x.duration_seconds||0).toFixed(1)} s · ${esc(x.style||'natural')} · ${Number(x.speed||1).toFixed(1)}×</small></div><div class="synth-history-text" title="${esc(x.text||'')}">${esc(x.text||'')}</div><div class="synth-history-actions"><button data-synth-play="${esc(x.id)}">▶</button><button data-synth-save="${esc(x.id)}">⇩</button><button class="danger" data-synth-delete="${esc(x.id)}">🗑</button></div></div>`).join(''):'<div class="status-box">Brak wygenerowanych plików.</div>'}catch(e){$('synthesisHistory').innerHTML=`<div class="status-box">${esc(e.message)}</div>`}
}
function renderABCompare(r){if(!$('abCompare'))return;const a=r.base?.item,b=r.trained?.item;if(!a||!b)return;$('abCompare').className='ab-pair';$('abCompare').innerHTML=`<div class="ab-item"><b>A · Bazowy XTTS v2</b><small>${Number(a.duration_seconds||0).toFixed(1)} s · ${esc(a.style||'natural')}</small><div class="action-row"><button class="btn btn-soft small" data-synth-play="${esc(a.id)}">▶ Odtwórz A</button><button class="btn btn-soft small" data-synth-save="${esc(a.id)}">⇩ Zapisz</button></div></div><div class="ab-item"><b>B · Wytrenowany XTTS</b><small>${Number(b.duration_seconds||0).toFixed(1)} s · ${esc(b.style||'natural')}</small><div class="action-row"><button class="btn btn-soft small" data-synth-play="${esc(b.id)}">▶ Odtwórz B</button><button class="btn btn-soft small" data-synth-save="${esc(b.id)}">⇩ Zapisz</button></div></div>`}
...
const synth=async dash=>{
  ...
  const style=dash?$('dashStyle').value:$('synthStyle').value,speed=Number(dash?$('dashSpeed').value:$('synthSpeed').value),model=dash?'active':$('synthModelMode').value,rvc=dash?false:$('synthRvc').checked,btn=dash?$('dashGenerate'):$('synthGenerate');
  try{btn.disabled=true;const old=btn.textContent;btn.dataset.old=old;btn.textContent='Generowanie…';const r=await api('synthesize',text,profile,lang,rvc,style,speed,model,'');$('playerStatus').textContent=r.name;$('synthResult').textContent=`Gotowe · ${r.model_mode==='trained'?'Wytrenowany':'Bazowy'} · ${r.path}`;toast('Synteza gotowa',r.name,'success');await loadSynthesisHistory();await refreshState()}catch(e){toast('Synteza',e.message,'error');$('synthResult').textContent=e.message}finally{btn.disabled=false;btn.textContent=btn.dataset.old||'Generuj'}
};
...
$('synthGenerateAB').onclick=async()=>{const b=$('synthGenerateAB');try{b.disabled=true;b.textContent='Generuję A/B…';const r=await api('synthesize_ab',$('synthText').value,$('synthProfile').value,$('synthLanguage').value,$('synthStyle').value,Number($('synthSpeed').value));renderABCompare(r);await loadSynthesisHistory();toast('A/B gotowe','Wygenerowano bazowy i wytrenowany model.','success')}catch(e){toast('Test A/B',e.message,'error')}finally{b.disabled=false;b.textContent='A/B Bazowy ↔ Wytrenowany'}};
$('refreshSynthHistory').onclick=loadSynthesisHistory;
