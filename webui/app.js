...
async function pollMusicInstall(){try{const st=await api('music_engine_install_status');$('musicEngineStatus').textContent=`${st.message||''} · ${st.progress||0}%`;if(['completed','error','needs_espeak'].includes(st.state)){clearInterval(musicInstallTimer);musicInstallTimer=null;toast('DiffRhythm',st.message,st.state==='completed'?'success':st.state==='error'?'error':'info');await refreshMusicEngineStatus()}}catch(e){}}
