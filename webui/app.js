...
async function runContinuousSession(){
  ...
  try{while(continuousSession.active){
    ...
    const r=await withRecordingTimer(duration,`Sesja ciągła · próbka ${continuousSession.count+1}`,text,()=>api('record_training_sample',profile,text,duration,Number($('trainDevice').value)),first?3:0);
    ...
    first=false;updateContinuousMeta('Przerwa 3 s · przygotuj się do kolejnej próbki');await waitMs(3000);
...
async function refreshMusicEngineStatus(){if(!$('musicEngineStatus'))return;try{const r=await api('music_engine_status');const es=r.espeak?.ok?'eSpeak OK':'brak eSpeak NG';$('musicEngineStatus').textContent=r.installed?`DiffRhythm zainstalowany · ${es}. ${r.espeak?.ok?'Możesz generować. Przy pierwszym renderze pobiorą się modele AI.':'Dokończ instalację eSpeak NG.'}`:`Najpierw zainstaluj DiffRhythm, a następnie eSpeak NG.`;$('musicEngineBadge').textContent=r.installed&&r.espeak?.ok?'DIFFRHYTHM READY':'KONFIGURACJA';$('musicEngineBadge').className=`status-pill ${r.installed&&r.espeak?.ok?'green':'neutral'}`;$('installEspeakNg')?.classList.toggle('hidden',!!r.espeak?.ok);if($('musicDiffStep')){$('musicDiffStep').textContent=r.installed?'Zainstalowany':'Wymaga instalacji';$('musicDiffStep').closest('.engine-step')?.classList.toggle('ready',!!r.installed)}if($('musicEspeakStep')){$('musicEspeakStep').textContent=r.espeak?.ok?'Zainstalowany':'Wymaga instalacji';const step=$('musicEspeakStep').closest('.engine-step');step?.classList.toggle('ready',!!r.espeak?.ok);step?.classList.toggle('error',!r.espeak?.ok)}if($('musicModelsStep')){$('musicModelsStep').textContent=r.installed&&r.espeak?.ok?'Automatycznie przy pierwszym renderze':'Najpierw przygotuj kroki 1–2'}if(r.install&&['starting','downloading','installing'].includes(r.install.state)){$('musicEngineStatus').textContent=`${r.install.message} · ${r.install.progress||0}%`}}catch(e){$('musicEngineStatus').textContent=e.message}}
...
let espeakInstallTimer=null;
async function pollEspeakInstall(){try{const st=await api('espeak_install_status');const box=$('espeakInstallStatus');if(box){box.classList.remove('hidden');box.textContent=`${st.message||''}${st.progress!=null?` · ${st.progress}%`:''}`;}if(['completed','error'].includes(st.state)){clearInterval(espeakInstallTimer);espeakInstallTimer=null;toast('eSpeak NG',st.message,st.state==='completed'?'success':'error');await refreshMusicEngineStatus();}}catch(e){}}
